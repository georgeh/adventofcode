const inputs = `nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
...
function isAnagram( first, second ) {
	...
	for ( let i = 0; i < first.length; i++ ) {
		secondIndex = secondArr.indexOf( firstArr[i] );
		if ( -1 === secondIndex ) {
			return false;
		}
	...
	return ( 0 === secondArr.length );
}

function noAnagrams( wordArr ) {
	for ( i = 0; i < ( wordArr.length - 1 ); i++ ) {
		for ( j = i+1; j < wordArr.length; j++ ) {
