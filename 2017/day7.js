...
let input = `uglvj (99) -> ymfjt, gkpgf
vvwrg (51)
qrpgt (5)
qhqbqj (55)
taxdaf (23)
...
const discs = input.split("\n").reduce( ( discMap, discLine ) => {
	const [ discInfo, childNames ] = discLine.split( " -> " );
	const [ name, weight ] = discInfo.split( " " );
	discMap[name] = {
		name,
		childNames,
...
Object.keys( discs ).forEach( ( name ) => {
	const disc = discs[ name ];
	if ( disc.childNames ) {
		disc.childNames.split( ', ' ).forEach( childName => {
			disc.children.push( discs[ childName ] );
			discs[ childName ].parent = disc;
		} )
	}
	lastName = name;
} )

function getParent( disc ) {
	if ( ! disc.parent ) {
		return disc;
	}
	return getParent( disc.parent );
}

const root = getParent( discs[ lastName ] )
console.log( root.name );

const add = ( a, b ) => a + b;
function towerWeight( disc ) {
	if ( ! disc.towerWeight ) {
		disc.towerWeight = disc.weight + disc.children.map( towerWeight ).reduce( add, 0 );
	}
	return disc.towerWeight;
}

function arrayIsSame( arr ) {
	let test = undefined;
	return arr.every( ( n ) => {
...
function isBalanced( disc ) {
	if ( 'undefined' !== typeof disc.isBalanced ) {
		return disc.isBalanced;
	}

	if ( 0 === disc.children.length ) {
		return true;
	}

	const childTowerWeights = disc.children.map( towerWeight );
	disc.isBalanced = arrayIsSame( childTowerWeights );
	return disc.isBalanced;
}

function oddOneOut( arr ) {
	if ( arr.length < 3 ) {
		throw "Cannot figure out which one is weird"
	}

	const counts = {};
	arr.forEach( n => counts[n] = counts[n] ? counts[n] + 1 : 1 )
	return arr.find( v => counts[v] === 1 );
}

function childrenAreBalanced( disc ) {
	if ( 0 === disc.children.length ) {
		return true;
	}

	return disc.children.every( isBalanced );
}

function normalValue( arr ) {
	if ( arr.length < 3 ) {
		throw "Cannot figure out which one is weird"
	}
...
function findWrongDisc( disc ) {
	if ( childrenAreBalanced( disc ) ) {
		const childTowerWeights = disc.children.map( towerWeight );
		const wrongTowerWeight = oddOneOut( childTowerWeights );
		return disc.children.find( d => d.towerWeight = wrongTowerWeight );
	}

	for ( let i = 0; i < disc.children.length; i++ ) {
		const child = disc.children[i];
		if ( isBalanced( child ) ) {
			continue;
		}

		let wrongChild = findWrongDisc( child );
		if ( wrongChild ) {
			return wrongChild;
		}
	}
...
const wrongDisc = findWrongDisc( root );
const siblingTowerWeights = wrongDisc.parent.children.map( towerWeight );
const wrongTowerWeight = oddOneOut( siblingTowerWeights );
const rightTowerWeight = normalValue( siblingTowerWeights );
const diff = rightTowerWeight - wrongDisc.towerWeight;
// console.log( "" + rightTowerWeight + " - " + wrongDisc.towerWeight + " = " + diff );
// console.log( "" + diff + " + " + wrongDisc.weight + " = " + (wrongDisc.weight + diff) );
console.log( wrongDisc.weight + diff );