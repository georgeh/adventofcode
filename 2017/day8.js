...
lines
.map( ( [ reg, inst, amount, _, compareReg, compare, compareAmt ] ) => [ reg, inst, parseInt( amount ), _, compareReg, compare, parseInt( compareAmt ) ] )
.forEach( ( [ reg, inst, amount, _, compareReg, compare, compareAmt ] ) => {
	// console.log( [ reg, inst, amount, _, compareReg, compare, compareAmt ] );
	try {
		if( instructions[ compare ]( compareReg, compareAmt ) ) {
			// console.log( inst )
			instructions[ inst ]( reg, amount );
			highWaterMark = Math.max( get( reg ), highWaterMark );
		}
	} catch ( e ) {
		console.log( [ reg, inst, amount, _, compareReg, compare, compareAmt ] );
		throw e;
...
const max = Object.keys( registers ).reduce( ( best, key ) => ( ( best > registers[ key ] ) ? best : registers[ key ] ), Number.NEGATIVE_INFINITY );
console.log( max );
