...
// presorted
input = `[1518-02-24 23:58] Guard #2347 begins shift
...
let guardsById = input.split( '\n' )
	.map( ( l ) => {
		let matches = /^\[(.*)\] (.*)/g.exec( l );
		return [ new Date( matches[ 1 ] ), matches[ 2 ] ];
	} )
	.reduce( ( state, [ timestamp, line ] ) => {
		let currentTuple = state.currentTuple;
		let guardMatch = /Guard #(\d+) begins shift/g.exec( line );
		if ( guardMatch ) {
			currentTuple[ 0 ] = guardMatch[ 1 ];
		} else if ( line == 'falls asleep' ) {
			currentTuple[ 1 ] = timestamp;
		} else if ( line == 'wakes up' ) {
			currentTuple[ 2 ] = timestamp;
			state.tuples.push( currentTuple );
			state.currentTuple = [ currentTuple[ 0 ] ];
		}
		return state;
	}, { tuples: [], currentTuple: [] } )
	.tuples
	.reduce( ( state, [ id, start, end ] ) => {
		if ( !state[ id ] ) {
			state[ id ] = { id, total: 0, minutes: {} };
		}
		const guard = state[ id ];
		for ( i = start.getMinutes(); i < end.getMinutes(); i++ ) {
			guard.total++;
...
const guards = Object.keys( guardsById ).map( id => guardsById[ id ] );

guards.forEach( guard => {
	guard.commonestMinute = Object.keys( guard.minutes ).reduce( ( commonest, currentMin ) => {
		return ( guard.minutes[ currentMin ] > guard.minutes[ commonest ] ) ? currentMin : commonest;
	} )
} )

const part1Guard = guards.reduce( ( sleepiest, current ) => {
	return ( current.total > sleepiest.total ) ? current : sleepiest;
} )

console.log(
	part1Guard.id, part1Guard.commonestMinute, parseInt( part1Guard.id ) * parseInt( part1Guard.commonestMinute )
);

const part2Guard = guards.reduce( ( mostPredictable, current) => {
	return ( current.minutes[ current.commonestMinute ] > mostPredictable.minutes[ mostPredictable.commonestMinute ] ) ? current : mostPredictable;
} )

console.log(
	part2Guard.id, part2Guard.commonestMinute, parseInt( part2Guard.id ) * parseInt( part2Guard.commonestMinute )
);
